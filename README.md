...
```bash
docker compose run --build --rm api npm test
```

`--build` matters: without it, `docker compose run` happily reuses whatever image it last built, even one from before your latest change — dependencies included.
